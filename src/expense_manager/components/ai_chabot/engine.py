...
public.processed_items(
  ...
  quantity integer,
  price real,                     -- price in Euros
  total real,                    -- total line price in Euros (if multiple quantity)
  discount real,
...
- "most_spent_item_purchase_dates":
    - If the user asks for the purchase dates / when purchased / dates for each purchase
    of the most spent item (optionally with store name),
    set intent="most_spent_item_purchase_dates" and needs_clarification=false.
...
- If the user asks for dates/when purchased, the SQL MUST select a date:
  COALESCE(receipt_date, created_at::date) AS purchase_date
...
- most_spent_item_purchase_dates:
  Use a CTE to find the top (item_text, shop_name) by total spend.
  Then return one row per purchase line with:
    purchase_date = COALESCE(receipt_date, created_at::date),
    shop_name,
    item_text,
    line_spend = COALESCE(total, price * COALESCE(quantity,1), 0)
...
def answer(message: str, conversation_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    answer_text = llm_summarize(user_text, intent_obj, rows)
